...
var NinjaGuru = (function(timing) {
  ...
  // use a single cache object to store everything
  // we don't want any leaks
  // and this keeps things organized, even if a bit verbose
  var cache = {
  ...
  function renderTextStat(label, value, unitLabel) {
    return [
      '<div style="display: inline-block; margin-right: 4em; font-size: 16px; padding: 20px; float: left;">',
        '<span style="font-weight: bold">' + label + ': </span>' + value + unitLabel +
      ' </div>'
    ].join('');
  }
  ...
  function seriouslyInit() {
    ...
    cache.dom.textStats.wrap.classList.add('ninjaguru-text');

    cache.dom.chartStats.wrap.id = cache.dom.bar.prefix + '-chartStats';
    cache.dom.chartStats.wrap.classList.add('ninjaguru-text')

    cache.dom.chartStats.fps.canvas.id = cache.dom.bar.prefix + '-fpsChart';
    cache.dom.chartStats.fps.canvas.classList.add("ninjaguru-canvas");
